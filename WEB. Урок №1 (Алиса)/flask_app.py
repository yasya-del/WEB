...
def handle_dialog(req, res):
    ...
    if req['session']['new'] or rabbit:
        ...
        res['response']['text'] = f'Привет! Купи {name.lower()}а!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = f'{name}а можно найти на Яндекс.Маркете!'
        if name == 'Слон':
            name = 'Кролик'
            rabbit = True
        else:
    ...
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {name.lower()}а!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={name.lower()}",
            "hide": True
        })
